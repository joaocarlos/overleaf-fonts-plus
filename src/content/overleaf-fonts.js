...
;(() => {
    ...
    function applyFontToEditor(fontCssFamily) {
        ...
        if (docs.length === 0) {
            debugLog(
                "No Ace editor found yet; will rely on CSS being picked up later.",
            )
        }
        ...
        docs.forEach((doc, index) => {
          ...
          .ace_editor *,
          .cm-scroller,
          .cm-scroller *:not(.review-mode-switcher-container):not(.review-mode-switcher-container *) {
        ...
        `
            ...
            window.setTimeout(() => {
                ...
                const resolved =
                    doc.defaultView.getComputedStyle(editor).fontFamily
                debugLog(
    ...
    function handleSelectChange(event) {
        ...
        if (custom) {
            ...
            chrome.storage.sync.set({
                "overleaf-fonts-plus-current": custom.id,
            })
        } else {
    ...
    function initOnceSettingsModalAppears() {
        ...
        if (immediateSelect) {
            ...
            debugLog(
                "fontFamily select already present, initialized immediately.",
            )
        }
    }
    ...
    function applyStoredFontOnLoad() {
        // Apply the saved font immediately when the editor becomes available,
        // without waiting for the settings modal to be opened.
        chrome.storage.sync.get("overleaf-fonts-plus-current", (data) => {
            const savedId = data["overleaf-fonts-plus-current"]
            if (!savedId) return
            ...
            // Wait for the editor to appear, then apply the font
            const EDITOR_SELECTOR = ".ace_editor, .cm-editor"

            function tryApply() {
                if (document.querySelector(EDITOR_SELECTOR)) {
                    applyFontToEditor(custom.cssFamily)
                    debugLog("Applied stored font on page load:", custom.label)
                    return true
                }
                return false
            }
            ...
            // Otherwise, observe for editor appearance
            const observer = new MutationObserver(() => {
                if (tryApply()) {
                    observer.disconnect()
                }
            ...
            // Safety timeout to stop observing after 30 seconds
